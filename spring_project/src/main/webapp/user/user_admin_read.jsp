...
<div id="minho_main">
	<div class="row container-fluid login">
		...
		<p>${where}</p>
			<div class="col-sm-6">
				<h2 class="mainAdminText">관리자 관리</h2>
			</div>
			<div class="col-sm-2">
				<a
					href="${pageContext.request.contextPath}/user/user_admin_create.user"
					title="관리자 등록" class="btn-block gumbb">등록하기</a>
			</div>
			<div class="col-sm-2">
				<select id="where" name="where" class="gum">
					<option value="default">조건별</option>
					<option value="1">등급순</option>
			...
			<div class="col-sm-2">
				<div class="dropdown">
					...
					<ul class="dropdown-menu">
						...
						<li><a id="yes" title="관리자 계정 활성화">활성화</a></li>
						<li><a id="no" title="관리자 계정 비활성화">비활성화</a></li>
					</ul>
				</div>
			</div>
			<table class="table table-striped tt">
				...
				<tbody>
					<c:forEach var="dto" items="${list}" varStatus="status">
						<tr>
							<td><c:if test="${dto.user_role_code == 2 }">
									<input type="checkbox" id="check" name="check"
										value="${dto.user_id}" />
								</c:if></td>
							<td><c:choose>
									<c:when test="${dto.user_role_code == 1 }">메인관리자</c:when>
									<c:otherwise>서브관리자</c:otherwise>
								</c:choose></td>
							<td><a
							...
							<td><c:choose>
									<c:when test="${dto.user_condition_code == 1}">
				활성화
				</c:when>
									<c:otherwise>
				비활성화
					...
					<ul class="pagination">
						<c:if test="${paging.start>= paging.bottomlist}">
							...
							<li class="page-item"><a class="page-link"
								href="${pageContext.request.contextPath}/user/user_admin_read.user?pstartno=${(paging.start-2)*paging.onepagelist}">이전</a>
							</li>
						</c:if>
						<c:forEach begin="${paging.start}" end="${paging.end}" var="i">
							...
							<li class="page-item"
								...
								<a class="page-link"
								href="${pageContext.request.contextPath}/user/user_admin_read.user?pstartno=${ (i-1)*paging.onepagelist}">${i}</a>
							</li>
						</c:forEach>
						...
						<c:if test="${paging.pagetotal> paging.end}">
							...
							<li class="page-item">
								...
								href="${pageContext.request.contextPath}/user/user_admin_read.user?pstartno=${(paging.end)*paging.onepagelist}">다음</a>
							</li>
						</c:if>
					</ul>
				</div>
...
$(document).ready(function() {
 ...
 function listResult(data) {
	    ...
	    $.each(data.data, function(idx, dto) {
	        ...
	        if (dto.user_role_code == 2) {
	            checkboxHtml = "<input type='checkbox' id='check' name='check' value='" + dto.user_id + "'>";
	            roleHtml = "서브관리자";
	        } else {
	            roleHtml = "메인관리자";
	        }

	        $tr.append($("<td>").html(checkboxHtml))
	            .append($("<td>").html(roleHtml))
